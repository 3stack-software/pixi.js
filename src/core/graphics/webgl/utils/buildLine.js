...
function buildLine(graphicsData, webGLData)
{
    ...
    const indexCount = points.length;
    ...
    for (i = 0; i < points.length; i += 2)
    {
        ...
        {
            middlePoints.push({ x: points[i], y: points[i + 1] });
        }
        else
        {
            middlePoints.push({
    ...
    {
        const p0 = middlePoints[i - 1];
        const p1 = { x: points[i * 2], y: points[(i * 2) + 1] };
        const p2 = middlePoints[i];
    ...
    // createRoundCap({ x: points[0], y: points[1] }, p00, p01, p02, vertices);
    // createRoundCap({ x: points[points.length - 2], y: points[points.length - 1] }, p10, p11, p12, vertices);
